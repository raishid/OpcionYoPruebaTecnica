...
import { updateDoctors } from "~/services/doctors";
...
definePageMeta({
  async validate(route) {
    ...
    const { error } = await useGetDoctor(id as string);
    if (error.value) {
      return false;
    }
...
const { data: doctor, error: _error } = await useGetDoctor(id as string);
...
const hours = computed(() => {
  ...
  return {
    start: new Date(`0001-01-01T${doctor.value.horaries.start}`),
    end: new Date(`0001-01-01T${doctor.value.horaries.end}`),
    lunch_start: new Date(`0001-01-01T${doctor.value.horaries.lunch_start}`),
  };
});
...
const formData = ref({
  ...
  time_zone: doctor.value?.time_zone,
  hour_start: hours.value.start,
  hour_end: hours.value.end,
  lunch_start: hours.value.lunch_start,
  days: doctor.value?.horaries.days,
} as storeDoctorData);
...
const onSubmit = async () => {
  ...
  const resp = await updateDoctors(parsedData, id as string);
